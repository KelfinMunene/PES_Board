...
int main()
{
    ...
    const float r_wheel = 0.070f / 2.0f;
    const float b_wheel = 0.170f;
    ...
    // Sensor Bar
    const float bar_dist = 0.07f; // Distance from wheel axis to sensorbar
    ...
    bool leftTurn = false, rightTurn = false, crossRoad = false, isIntersection = false , uTurn = false;
    int dir = 0;

    float finalAngle = 0;
    float fixedAngle = 0;
    float get_pos_uturn1 =0;
    float get_pos_uturn2 =0;
    ...
    while (true)
    {
        ...
        if (do_execute_main_task)
        {
            ...
            printf("left mean: %f right mean: %f center mean: %f \n", leftMean,rightMean,centerMean);
            ...
            case RobotState::TRAIN:
            {
                ...
                case MotionState::FOLLOWLINE:
                {
                    ...
                    uTurn = (leftMean < 0.001f && centerMean < 0.001f && rightMean < 0.001f);
                    ...
                    if (isIntersection)
                    {
                        ...
                        fixedAngle = motor_M1.getRotation() + 0.0f;
                        finalAngle = motor_M2.getRotation() + 0.0f;
                    ...
                    else
                    {
                        ...
                        const float w = Kp * angle +  tanh_approx(kp_nl *angle);
			            const float  v = (0.5f * wheel_vel_max * r_wheel)*(1- tanh_approx(kp_nl*fabsf(angle)));
                        robot_coord = {v, w};
                ...
                case MotionState::PREPARE_TURN:
                {
                    ...
                    if (std::fabs(motor_M1.getRotation() - fixedAngle) < 0.01)
                    {
                        ...
                        {
                            // 180 DEGREE TURN
                            get_pos_uturn1 = motor_M1.getRotation();
                            get_pos_uturn2 = motor_M2.getRotation();
                            finalAngle = motor_M1.getRotation() + getrevolutions(90, r_wheel, b_wheel);
                            fixedAngle = motor_M2.getRotation() + getrevolutions(-90, r_wheel, b_wheel); // fixed
                ...
                case MotionState::LEFT_TURN:
                {
                    ...
                    if (std::fabs(motor_M1.getRotation() - finalAngle) < 0.1)
                    {
                        motion_state = MotionState::FOLLOWLINE;
                    }
                ...
                case MotionState::RIGHT_TURN:
                {
                    ...
                    if (std::fabs(motor_M2.getRotation() - finalAngle) < 0.1)
                    {
                        motion_state = MotionState::FOLLOWLINE;
                    }
                    break;
                ...
                {   
                    motor_M1.setRotation(get_pos_uturn1);
                    motor_M2.setRotation(get_pos_uturn2);
